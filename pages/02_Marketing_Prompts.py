...
prompt_options = {
    "Summarize": "Read the content above slowly and take your time to understand it. Think through carefully step by step. Extract the key insights and takeaways a few paragraphs at a time instead of looking at everything. Write in point form and organize section in headers. Let the bullet points flow and avoid writing too short because the context is lost and you may end up making the content even harder to understand. Make sure it is comprehensive and complete and you don't lose out important information. Note that you should not be writing an outline, I am learning from the content",
    ...
    "Find Value Proposition": "Looking at the information above. Identify a likely user then fill in the details from the text above with each section with what the user is trying to do. Identify multiple things they are trying to do (use case) so it has to be addressed with all the points below 1. A user 2. What are they trying to do (start with the phrase “they are trying to [their end goal or jobs to be done]) 3. How they are doing it (current flawed or less superior older way; start with the word “by”)  (multiple) Then for every subpoint in 'how they are doing it': 4. Problem (blocker of progress; to be addressed by benefit later; start with 'which leads to...' Usually deterioration/consequence of the current way described with a adverb or adjective) 5. Limitation of current way (addressed by product capability; start with the word 'because…' the reason and root cause of the problem caused by a specific step in the current way. Be specific and descriptive here! ) 6. Product capability (addresses limitation of current way; start with the phrase 'now you can…' a descriptive activity that the solutions enable to address points in limitation of current way) 7. Product feature (few words on what the capability is called. Start with the word 'using'. Also explain how it works and steps it performs to achieve the capability. Be very detailed step by step) 8. Benefit (addresses problem; usually an improvement on something described with an adverb or adjective; starts with the phrase so that [the user]…) 9. Add a section: Why do this (this is how it contributes to their use case; start with the phrase “in order to”) 10. Determine if its a vitamin or painkiller by assessing if the user, when faced without the solution, will greatly suffer or would simply find working alternatives or not solve the problem at all Section them nicely in headers. Note that you do not come up with ideas or solutions but merely extract information from the text. If there isn't, then report a 'blank'",
