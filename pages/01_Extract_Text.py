...
def extract_text_from_pdf(pdf_reader, start_page, end_page):
    ...
    for page_num in range(start_page, end_page + 1):
        ...
        progress_text.text(
            f"Progress: {progress:.0%} ({page_num + 1}/{end_page + 1})")
...
def extract_text_from_image(image_bytes):
    try:
        with st.spinner("Extracting text from image..."):
            ...
            # Downsize the image if it's larger than 720p (optional, but
            # recommended)
...
st.title("Copy Pasta 🍝")
st.subheader(
    "Copy text from articles, PDFs, and images for LLM prompting with one click"
)
    ...
    if pdf_file:
        ...
        if not all_pages:
            start_page = st.number_input(
                "Starting Page Number", min_value=1, step=1)
            end_page = st.number_input(
                "Ending Page Number", min_value=1, step=1)
...
if "main_text_with_prefix" in st.session_state:
    st_copy_to_clipboard(
        st.session_state["main_text_with_prefix"], "Copy Extracted Text"
    )
